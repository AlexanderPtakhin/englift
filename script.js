...
// REMOVED: import idiomsBankData from './idioms/idioms.json';
// =============================================
...
// REMOVED: dirtyIdiomIds
let deletedWordIds = new Set();
// REMOVED: deletedIdiomIds
...
let weeklyReviewDetail = []; // [{ date: "2025-05-11", words: 0 }] // REMOVED: idioms
...
function updateWeeklyReviewDetail(type) {
  ...
  if (!day) {
    day = { date: today, words: 0 }; // REMOVED: idioms
    weeklyReviewDetail.push(day);
  }
  if (type === 'words') day.words++; // REMOVED: idioms type
...
// REMOVED: window.idiomsBank - now using WordBankDB (IndexedDB)
function markWordDirtyForCache(wordId) {
  dirtyWordIds.add(wordId);
  scheduleCacheSave();
}
// REMOVED: markIdiomDirtyForCache
// REMOVED: markIdiomDeletedForCache
...
async function loadFromCache() {
  try {
    ...
    console.log('[CACHE] Загружено из кеша:', window.words.length, 'слов');
...
window.DEBUG = false;
// REMOVED: window.idioms = []; // глобальный массив идиом (только пользовательские)
...
// REMOVED: selectedIdiomExercises
...
// REMOVED: currentFilteredIdioms
let renderedWordsCount = 0; // количество отрендеренных слов в текущей сетке
// REMOVED: renderedIdiomsCount
// REMOVED: Idioms variables (idiomsVisibleLimit, idiomsIsLoadingMore, etc.)
...
// REMOVED: lastFetchedIdiomData
let pendingWordUpdates = new Map();
// REMOVED: pendingIdiomUpdates
let wordSyncTimer;
// REMOVED: idiomSyncTimer
...
const BADGES_DEF = [
  ...
  // REMOVED: idioms badges (first_idiom, idiom_collector_25, idiom_collector_100)
  // ===== Прогресс изучения =====
  {
    id: 'first_learned',
  ...
  },
  {
    ...
    name: 'Серия обучения',
    description: 'Изучите 3 слова подряд',
  ...
  },
  {
    ...
    name: 'Горячая серия',
    description: 'Изучите 10 слов подряд',
    icon: 'whatshot',
...
// REMOVED: window.pendingIdiomUpdates
// =============================================
...
// REMOVED: window.normalizeIdiom = normalizeIdiom;
...
// REMOVED: window.speakIdiom function
...
// REMOVED: updateIdiomsCount function
// ============================================================
...
// REMOVED: markIdiomDirty, scheduleIdiomSync, syncPendingIdioms functions
...
function renderWeekChart() {
  ...
  container.querySelectorAll('#activity-week-view, #activity-month-view').forEach(el => el.remove());
  // Рендерим только тепловую карту
...
function renderMonthHeatmap(container) {
  ...
  for (let i = 34; i >= 0; i--) {
    ...
    const total = stats ? stats.words ?? 0 : 0;
    let level = 0;
    if (total > 0) level = 1;
    if (total >= 50) level = 2;
    if (total >= 80) level = 3;
...
function showFeedback({
  ...
  // REMOVED: isIdiom check - always use word.en/word.ru
  let wordEn = word?.en ?? '';
  ...
  const wordRu = parseAnswerVariants(word?.ru ?? '').join(', ');
...
/**
 * Универсальная SRS-логика для слов.
 * Заменяет updStats.
 */
...
function checkBadges(perfectSession = false) {
  ...
  // REMOVED: const totalIdioms = window.idioms ? window.idioms.length : 0;
  const learnedWords = window.words
    ? window.words.filter(w => w.stats?.learned).length
    : 0;
  // REMOVED: const learnedIdioms = window.idioms
...
function getBadgeProgress(def) {
  ...
  const valMap = {
    ...
    // REMOVED: idioms: window.idioms ? window.idioms.length : 0,
    learned: window.words ? window.words.filter(w => w.stats?.learned).length : 0,
    // REMOVED: learnedIdioms: window.idioms ? window.idioms.filter(i => i.stats?.learned).length : 0,
    streak: streak.count,
    level: xpData.level,
    dailyGoalsCompleted: window.dailyGoalsCompleted || 0,
    perfectSessions: window.perfectSessions || 0,
  ...
  const targets = {
    ...
    // REMOVED: idiom targets (first_idiom, idiom_collector_25, idiom_collector_100)
...
function getProgressText(progress) {
  switch (progress.type) {
    ...
    case 'learningStreak': return pluralize(progress.remaining, 'слово', 'слова', 'слов') + ' подряд';
...
function updateDueBadge() {
  ...
  // REMOVED: idiomsLearning and idiomsDue calculations
  ...
  if (desktopBadge) {
    ...
    desktopBadge.title = `Слов: ${wordsDue} (фразы и идиомы не входят в этот счётчик)`;
  }
  if (mobileBadge) {
    mobileBadge.textContent = displayDue;
    mobileBadge.style.display = totalDue > 0 ? 'flex' : 'none';
    mobileBadge.title = `Слов: ${wordsDue} (фразы и идиомы не входят в этот счётчик)`;
  }
  ...
  if (learningPill) {
    const learningCount = wordsLearning; // REMOVED: idioms mode check
  ...
  if (pc) {
    ...
    const wordsTotalText = isTinyScreen
      ...
      : isSmallScreen
        ? `${wordsTotal}`
        : `${wordsTotal} всего`;
    // REMOVED: idiomsLearnedText and idiomsTotalText
    pc.innerHTML = `
      <div class="spc-card">
...
async function syncAfterReconnect() {
  ...
  // REMOVED: console.log('[SYNC RECONNECT] Local idioms:', window.idioms?.length);
  console.log(
    '[SYNC RECONNECT] Pending word updates:',
    pendingWordUpdates.size,
  );
  // REMOVED: console.log('[SYNC RECONNECT] Pending idiom updates:', pendingIdiomUpdates.size);
  // Снимок локального состояния до синка - защита от потери данных
  const localSnapshot = [...(window.words || [])];
  // REMOVED: const idiomSnapshot = [...(window.idioms || [])];
  try {
...
// REMOVED: Custom select functionality for idioms (idiomCustomSelect, idiomCustomTrigger, etc.)
// REMOVED: Filter pills for idioms
document.getElementById('words-grid')?.addEventListener('click', e => {
...
// REMOVED: Обработчик кликов для карточек идиом (and its remnants)
// Delete modal
let pendingDelId = null;
let pendingDeleteType = 'word';
...
// REMOVED: Search for idioms (idioms-search event listener)
document.getElementById('del-confirm').addEventListener('click', () => {
  if (pendingDelId) {
    ...
    // REMOVED: pendingDeleteType === 'idiom' branch
...
// REMOVED: function startEditIdiom
// ============================================================
// ADD WORDS
// ============================================================
document.getElementById('single-form')?.addEventListener('submit', async e => {
  ...
  if (success) {
    ...
    // Не переключаемся на словарь, оставляемся на текущей вкладке
    // switchTab('words');
    setTimeout(() => {
...
// REMOVED: Обработчик формы добавления идиом
...
// REMOVED: Все остатки автодополнения идиом (idiomEnInput, idiomSuggestionsContainer, selectedIdiomSuggestionIndex, updateSelectedIdiomItem)
// File import variables
  ...
  showConfirmModal(
    ...
    async () => {
      try {
        ...
        // REMOVED: idiom deletion from server
        // 4. Очищаем IndexedDB
        await clearLocalWordCache();
        // 5. Очищаем очереди синхронизации
        pendingWordUpdates.clear();
        // REMOVED: pendingIdiomUpdates.clear();
        // 5. Обновляем интерфейс
        refreshUI();
        // REMOVED: renderIdioms(); // обновляем список идиом
        // REMOVED: updateIdiomsCount(); // обновляем счётчик
        // Обновляем счётчики в профиле
        markProfileDirty('total_words', 0);
        // REMOVED: markProfileDirty('total_idioms', 0);
        markProfileDirty('learned_words', 0);
        toast('✅ Весь словарь (слова) очищен!', 'success');
      } catch (error) {
...
document.querySelectorAll('.chip[data-mode]').forEach(c =>
  c.addEventListener('click', () => {
    ...
    // REMOVED: selectedIdiomExercises.length = 0;
    ...
    // REMOVED: idioms mode exercise grid
    if (practiceMode === 'verbs') {
...
function updateExerciseSelection() {
  const mode = practiceMode; // 'normal' или 'verbs'
  const selectedArray =
    mode === 'verbs'
...
function startSession(cfg) {
  ...
  try {
    ...
    // 2. Формируем пул данных (слова или глаголы)
    ...
    window.session = {
      ...
      exTypes,
      dir: dirVal,
      mode: practiceMode, // 'normal' или 'verbs'
      dataType: practiceMode === 'verbs' ? 'verbs' : 'words',
    };
...
function showResults() {
  ...
  // REMOVED: const isIdiom = practiceMode === 'idioms';
  // ── Время сессии ─────────────────────────────────────────────
  ...
  const wrongChips = sResults.wrong
    .map((item, i) => {
      const word = esc(item.en);
      const trans = esc(parseAnswerVariants(item.ru).join(' / '));
      return `
  ...
  // ── Тип сессии ────────────────────────────────────────────────
  const sessionTypeLabel = window.session?.mode === 'exam'
    ? 'Экзамен'
    : 'Слова';
  // ── Блок ошибок / перфект ─────────────────────────────────────
  const wrongSection =
...
function nextExercise() {
  ...
  try {
    ...
    // REMOVED: const isIdiom = window.session.dataType === 'idioms';
    ...
    if (t === 'flash') {
            ...
            <div class="flashcard-inner" id="fc-inner">
              ...
              <div class="card-face back">
                <div style="display:flex;align-items:center;gap:${isVerb ? '1.5rem' : '.75rem'};justify-content:center">
                  ...
                  ${backWord === w.en ? `<button class="btn-audio" id="fc-audio-btn-back" title="Proounce"><span class="material-symbols-outlined">volume_up</span></button>` : ''}
                </div>
              </div>
            </div>
          </div>
      ...
      // REMOVED: isIdiom audio handlers
      if (frontWord === w.en) {
  ...
  <div class="flash-answer-btns">
    ...
    <button class="btn-pill btn-pill--secondary" id="knew-btn">
      ...
      let dataSource = window.words; // REMOVED: isIdiom check
      // Для слов используем весь банк для лучшего подбора дистракторов
      let bankPool = (window.wordBank && window.wordBank.length > 0)
        ? window.wordBank
        : [];
      ...
      let distractorCandidates = distractorItems
        .filter(x => {
          ...
          const enMatch = x.en && x.en.toLowerCase() === currentEn;
          const ruMatch = x.ru && x.ru.toLowerCase() === currentRu;
          if (enMatch || ruMatch) {
            return false;
          }
          // Исключаем аббревиатуры из дистракторов, если правильный ответ не аббревиатура
          if (x.en && x.en.length <= 4 && x.en === x.en.toUpperCase() && x.en !== x.en.toLowerCase()) {
      ...
      if (distractorCandidates.length < 3) {
        ...
        for (const x of remainingCandidates) {
          ...
          // Исключаем аббревиатуры из дистракторов, если правильный ответ не аббревиатура
          if (x.en && x.en.length <= 4 && x.en === x.en.toUpperCase() && x.en !== x.en.toLowerCase() && !currentIsAbbr) {
            continue;
          }
          let trans = isRUEN ? x.en : x.ru;
          const variants = parseAnswerVariants(trans);
      ...
      // Автоозвучка для EN→RU (не для глаголов)
      if (autoPron && !isRUEN && speechSupported && !isVerb) {
        safeExerciseAudio(() => window.speakWord(w));
      }
      if (exContent) {
        exContent.innerHTML = `
  <div class="ta-card">
    ...
    <div class="ta-submit-row">
      ...
      if (!isRUEN && speechSupported && !isVerb) {
...
(async () => {
  ...
  // REMOVED: renderRandomBankIdiom(); // для идиом
})();
renderWords();
renderXP();
renderBadges();
...
function runContextExercise(item, onComplete, exerciseType) {
  ...
  shuffledOptions.forEach(option => {
    ...
    btn.textContent = option.en.toLowerCase();
...
window.clearUserData = async function (isExplicitLogout = false) {
  ...
  // REMOVED: updateIdiomsCount();
  // Очистка IndexedDB
  try {
    await clearAllWords();
    // REMOVED: await clearAllIdioms();
  } catch (error) {
    console.error('❌ Ошибка очистки IndexedDB в clearUserData:', error);
  }
  dirtyWordIds.clear();
  // REMOVED: dirtyIdiomIds.clear();
  deletedWordIds.clear();
  // REMOVED: deletedIdiomIds.clear();
...
function openStoryModal(story) {
  ...
  // Добавляем скролл-поведение для модального окна
  const modalBox = modal.querySelector('.modal-box');
  if (modalBox) {
    let modalScrollY = modalBox.scrollTop;
    const SCROLL_THRESHOLD = 20;

    const handleModalScroll = () => {
      const currentScrollY = modalBox.scrollTop;
      const delta = currentScrollY - modalScrollY;
      const floatingWordBtn = document.getElementById('floating-add-word-btn');

      if (Math.abs(delta) > SCROLL_THRESHOLD && floatingWordBtn) {
        if (delta > 0) {
        ...
        modalScrollY = currentScrollY;
      }
    };

    modalBox.addEventListener('scroll', handleModalScroll);
  ...
  closeBtn.onclick = () => {
    ...
    const modalBox = modal.querySelector('.modal-box');
    if (modalBox && modal._scrollHandler) {
      modalBox.removeEventListener('scroll', modal._scrollHandler);
...
// REMOVED: Обработчики для модального окна добавления идиом (addIdiomModal, etc.)
...
// REMOVED: addIdiomBtn, emptyAddBtn, addIdiomModalClose, addIdiomModal event listeners
  ...
  try {
    ...
    await new Promise(resolve => {
      window.authExports?.loadWordsOnce(remoteWords => {
        ...
        // REMOVED: if (window.renderIdioms) window.renderIdioms();
    ...
    // 4. Переключаемся на вкладку историй после полной загрузки
    switchTab('stories');
    ...
    // REMOVED: renderIdioms after load
...
function updateFloatingButtonsVisibility() {
  ...
  else if (activeTab === 'tab-stories') activeBtn = floatingWordBtn;
...
function updateFloatingButtonsForTab(tabName) {
    ...
    if (tabName === 'words') {
        ...
        // Для историй показываем кнопку добавления слов
        if (floatingWordBtn) {
            floatingWordBtn.style.display = 'flex';
            floatingWordBtn.classList.remove('fab-hidden');
        }
...
setTimeout(() => {
  updateFloatingButtonsForTab('stories');
}, 100);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Показываем плавающую кнопку после загрузки
  if (floatingWordBtn) {
    floatingWordBtn.style.display = 'flex';
  }
...
(function initPracticeBottomSheet() {
  ...
  window.updateSummary = function updateSummary() {
    ...
    // Используем разные лейблы для глаголов и слов
    let labels;
    if (practiceMode === 'verbs') {
...
function showComparisonModal(data, friendName) {
  ...
  modal
    ...
    .forEach(badge => {
      badge.addEventListener('click', async e => {
        ...
        // REMOVED: idiom branch from missing words modal
      });
    });
  ...
  modal.querySelector('#add-missing-words').onclick = async () => {
    ...
    // REMOVED: for (const i of data.idioms.missing) { ... }
...
// REMOVED: window.addIdiom = addIdiom;
...
initTheme();
setupThemeToggle();
// switchTab('stories') - перенесен в onProfileFullyLoaded после загрузки данных
...
// REMOVED: window.testFullRenderIdioms
