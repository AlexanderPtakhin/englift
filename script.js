...
async function openChatWithFriend(friendId) {
  ...
  // Если это мобильное устройство, включаем полноэкранный режим СРАЗУ в начале
    ...
    if (fpanelChat) {
      ...
      requestAnimationFrame(() => {
        ...
        if (messageInput) {
          ...
          // Улучшенная обработка фокуса с учетом клавиатуры
          ...
          // Обработчик изменения размера окна (появление/скрытие клавиатуры)
          const handleResize = () => {
          ...
          fpanelChat._resizeHandler = handleResize;
  ...
  header.innerHTML = `
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; width:100%">
      ...
      <div style="display:flex; gap:8px; align-items:center">
        ...
        <button class="btn-icon" id="close-chat-btn" title="Закрыть чат">
    ...
    chatUpdateInterval = setInterval(async () => {
      if (window.currentChatFriend) {
        const messages = await getMessages(
          ...
          window.currentChatState?.messagesPerPage || 50,
  ...
  document.getElementById('clear-chat-btn').onclick = () => {
    ...
    modal.innerHTML = `
      ...
      <div class="modal-box" style="max-width: 400px;">
        ...
        <div class="modal-header">
          ...
          <h3><span class="material-symbols-outlined">delete</span> Очистить чат</h3>
        ...
        <div class="modal-body">
          ...
          <p>Удалить все сообщения в чате с <b>${esc(friend.username)}</b>?</p>

          <p style="color: var(--muted); font-size: 0.9rem">Это действие нельзя отменить.</p>

        </div>

        <div class="modal-actions">

          <button class="btn-pill btn-pill--secondary" id="confirm-clear">

            <span class="material-symbols-outlined">delete</span> Удалить

          </button>

          <button class="btn-pill btn-pill--secondary" id="cancel-clear">
    ...
    modal.querySelector('#cancel-clear').onclick = closeModal;
    ...
    modal.querySelector('#confirm-clear').onclick = async () => {
      try {
        const { error } = await supabase
          ...
          .or(
            `sender_id.eq.${window.currentUserId},receiver_id.eq.${window.currentUserId}`,
          )
        ...
        if (error) {
          ...
          toast('Ошибка при очистке чата', 'danger');
        } else {
  ...
  window.currentChatState = {
    ...
    hasMoreMessages: messages && messages.length >= messagesPerPage, // Если вернуло ровно 50, могут быть еще
  ...
  const updateReactionLocally = (messageId, emoji, change) => {
    ...
    if (reactionEl) {
      if (change <= 0) {
        reactionEl.remove(); // удаляем реакцию
      }
    } else if (change === 1) {
      ...
      newReaction.innerHTML = `<span>${emoji}</span>`;
  ...
  // Добавляем кнопку загрузки старых сообщений если есть еще сообщения

  const loadMoreHtml = window.currentChatState.hasMoreMessages
    ? `
  ...
  // Обработчик кнопки загрузки старых сообщений
  ...
  if (loadMoreBtn) {
    loadMoreBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const newOffset = state.currentOffset + state.messagesPerPage;
        ...
        if (olderMessages && olderMessages.length > 0) {
          ...
          // Вставляем старые сообщения после кнопки загрузки (перед текущими сообщениями)

          const loadMoreContainer = document.querySelector(
            '.load-more-messages',
          );

          const firstMessage = messagesList.querySelector('.chat-message');

          if (firstMessage) {
            firstMessage.insertAdjacentHTML('beforebegin', olderMessagesHtml);
          } else {
            loadMoreContainer.insertAdjacentHTML('afterend', olderMessagesHtml);
          }

          // Мгновенная коррекция скролла без видимых прыжков
          messagesList.style.scrollBehavior = 'auto'; // отключаем анимацию
          const newScrollHeight = messagesList.scrollHeight; // форсим reflow синхронно
          messagesList.scrollTop =
            oldScrollTop + (newScrollHeight - oldScrollHeight);
          requestAnimationFrame(() => {
            messagesList.style.scrollBehavior = ''; // возвращаем smooth обратно
          });
          ...
          state.hasMoreMessages =
            olderMessages.length === state.messagesPerPage;
          ...
          // Вешаем обработчики на новые сообщения

          const newMessages = messagesList.querySelectorAll(
            '.chat-message:not([data-processed])',
          );
        ...
        toast('Ошибка загрузки сообщений', 'danger');
      } finally {
  ...
  // Обработчик фокуса для клавиатуры
  const messageInput = document.getElementById('chat-message-input');
  if (messageInput) {
    messageInput.addEventListener('focus', () => {
      // Ждём появления клавиатуры (обычно 200-300ms)
      setTimeout(() => {
