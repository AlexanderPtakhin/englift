...
async function loadFromCache() {
  try {
    const [words] = await Promise.all([
      ...
      // REMOVED: window.UserDataCache.getAllIdioms(),
    ]);
    window.words = words.length ? words.map(normalizeWord) : [];
    // REMOVED: window.idioms = idioms.length ? idioms.map(normalizeIdiom) : [];
...
function renderWeekChart() {
  ...
  // REMOVED: const totalIdioms = days.reduce((s, d) => s + d.idioms, 0);
  ...
  weekView.querySelectorAll('.week-stat-item').forEach((item, idx) => {
    item.addEventListener('click', () => {
      ...
      detail.textContent = `${dayInfo.dayName}, ${dayInfo.dateShow}: слова — ${dayInfo.words}`; // REMOVED: idioms
    });
  });
  updateStreakBadge();
